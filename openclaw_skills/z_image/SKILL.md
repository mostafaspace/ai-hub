...
```json
{
  ...
  "data": [
      {
          "url": "http://192.168.1.26:9000/v1/images/outputs/gen_...png"
      }
  ]
}
```
...
Using the `url` returned in Step 2, simply make a standard HTTP `GET` request to it (e.g., `http://192.168.1.26:9000/v1/images/outputs/gen_...png`). The response body will be the raw `image/png` binary file. Save it to a local temporary file and attach it to your message.
