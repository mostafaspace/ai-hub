...
try:
    from ltx_core.quantization.fp8_cast import inject_weight_scales as _inject_fp8_scales
except ImportError:
    _inject_fp8_scales = None
...
class SingleGPUModelBuilder(Generic[ModelType], ModelBuilderProtocol[ModelType], LoRAAdaptableProtocol):
    ...
    def build(self, device: torch.device | None = None, dtype: torch.dtype | None = None) -> ModelType:
        ...
        if not lora_strengths or (min(lora_strengths) == 0 and max(lora_strengths) == 0):
            ...
            # inject_weight_scales() uses the raw (pre-cast) state dict to register them as buffers,
            # enabling the _replace_fwd_with_upcast forward hook to scale FP8 weights correctly.
            if _inject_fp8_scales is not None:
                _inject_fp8_scales(meta_model, raw_sd)
