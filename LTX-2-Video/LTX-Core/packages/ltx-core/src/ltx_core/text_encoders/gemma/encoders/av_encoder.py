...
class AVGemmaTextEncoderModelConfigurator(ModelConfigurator[AVGemmaTextEncoderModel]):
    ...
    def from_config(cls: type["AVGemmaTextEncoderModel"], config: dict) -> "AVGemmaTextEncoderModel":
        ...
        # HOTFIX: The unquantized Gemma-3 model requires 'rope_local_base_freq' and 'rope_theta'
        if not hasattr(gemma_config.text_config, "rope_local_base_freq"):
            gemma_config.text_config.rope_local_base_freq = 10000.0
        
        # HuggingFace rope_utils will look for rope_theta as well
        if getattr(gemma_config.text_config, "rope_theta", None) is None:
            gemma_config.text_config.rope_theta = 1000000.0
...
def create_and_populate(module: AVGemmaTextEncoderModel) -> AVGemmaTextEncoderModel:
    ...
    # Safely extract rope_type from either dict, object, or None
    try:
        if isinstance(config.rope_scaling, dict):
            rope_type = config.rope_scaling.get("rope_type", "linear")
            factor = config.rope_scaling.get("factor", 8.0)
        else:
            rope_type = getattr(config.rope_scaling, "rope_type", "linear")
            factor = getattr(config.rope_scaling, "factor", 8.0)
    except Exception:
        rope_type = "linear"
        factor = 8.0
        
    # HuggingFace transformers 5.3 strictly expects rope_scaling to be a dict
    original_rope_scaling = getattr(config, "rope_scaling", None)
    config.rope_scaling = {"rope_type": rope_type, "factor": factor}
    ...
    # Ensure everything goes to CPU device. If it stays as 'meta', PyTorch 2.x .to() will crash.
    target_device = "cpu"
    
    positions_length = len(v_model.embeddings.position_ids[0])
    position_ids = torch.arange(positions_length, dtype=torch.long, device=target_device).unsqueeze(0)
    v_model.embeddings.register_buffer("position_ids", position_ids)
    embed_scale = torch.tensor(model.config.text_config.hidden_size**0.5, device=target_device)
    l_model.embed_tokens.register_buffer("embed_scale", embed_scale)
    
    if hasattr(l_model, "rotary_emb_local"):
        l_model.rotary_emb_local.register_buffer("inv_freq", local_rope_freqs.to(target_device))
        
    if hasattr(l_model, "rotary_emb"):
        # For older versions of transformers
        if hasattr(l_model.rotary_emb, "inv_freq"):
            l_model.rotary_emb.register_buffer("inv_freq", inv_freqs.to(target_device))
        
        # For transformers 5.30+
        if hasattr(l_model.rotary_emb, "sliding_attention_inv_freq"):
            l_model.rotary_emb.register_buffer("sliding_attention_inv_freq", local_rope_freqs.to(target_device))
            l_model.rotary_emb.register_buffer("sliding_attention_original_inv_freq", local_rope_freqs.to(target_device))
        
        if hasattr(l_model.rotary_emb, "full_attention_inv_freq"):
            l_model.rotary_emb.register_buffer("full_attention_inv_freq", inv_freqs.to(target_device))
            l_model.rotary_emb.register_buffer("full_attention_original_inv_freq", inv_freqs.to(target_device))
