...
async def _ensure_initialized(app: FastAPI, model_idx: int = 1) -> None:
    """Lazily initialize models if they are not currently loaded."""
...
async def _ensure_llm_ready(app: FastAPI, lm_model_path_override: Optional[str] = None, backend_override: Optional[str] = None) -> None:
    ...
    async with app.state._llm_init_lock:
        ...
        if initialized:
            app.state.last_used["llm_handler"] = time.time()
            return

        print(f"[API Server] Lazily initializing LLM handler...")
        project_root = _get_project_root()
        checkpoint_dir = os.path.join(project_root, "checkpoints")
        
        lm_model_path = (lm_model_path_override or os.getenv("ACESTEP_LM_MODEL_PATH") or "acestep-5Hz-lm-0.6B").strip()
        backend = (backend_override or os.getenv("ACESTEP_LM_BACKEND") or "vllm").strip().lower()
        ...
        # Use handler's dtype as reference if possible
        ref_dtype = "float16" # fallback
        if app.state.handler and hasattr(app.state.handler, "dtype"):
            ref_dtype = app.state.handler.dtype

        status, ok = app.state.llm_handler.initialize(
        ...
        if not ok:
            ...
            raise RuntimeError(f"LLM initialization failed: {status}")
        else:
...
def create_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        # Idle tracking and auto-unload setup (10-minute timeout)
        app.state.last_used = {
            "handler": time.time(),
            "handler2": time.time(),
            "handler3": time.time(),
        ...
        async def _run_one_job(job_id: str, req: GenerateMusicRequest) -> None:
            ...
            if require_llm_req or want_llm_req:
                try:
                    ...
                    if require_llm_req:
                        print(f"[API Server] Job {job_id}: Required LLM initialization failed: {e}")
                    else:
                        print(f"[API Server] Job {job_id}: Optional LLM initialization failed: {e}")
            ...
            def _blocking_generate() -> Dict[str, Any]:
                ...
                if req.use_format and (caption or lyrics):
                    if not llm_available:
                        raise RuntimeError(f"5Hz LM init failed (needed for format): {getattr(app.state, '_llm_init_error', 'Unknown error')}")
        ...
        # Models will be lazily loaded on first request
        print("[API Server] Lazy loading enabled. Models will initialize on demand.")

        # Set up cleanup on exit
        def _cleanup_on_exit():
            print("[API Server] Cleaning up models on exit...")
            if getattr(app.state, "_initialized", False): app.state.handler.unload()
            if getattr(app.state, "_initialized2", False): app.state.handler2.unload()
            if getattr(app.state, "_initialized3", False): app.state.handler3.unload()
            if getattr(app.state, "_llm_initialized", False): app.state.llm_handler.unload()

        # Start background tasks
        app.state._idle_monitor_task = asyncio.create_task(_idle_monitor())
        ...
        if hasattr(app.state, "_idle_monitor_task"):
            app.state._idle_monitor_task.cancel()
    ...
    async def get_audio(path: str, request: Request, _: None = Depends(verify_api_key)):
        ...
        # Handle double-wrapped URLs (e.g., path="/v1/audio?path=...")
        if path.startswith("/v1/audio?path="):
            path = path.split("path=", 1)[1]
        
        # Decode path (handles cases where it was double-encoded or contains encoded separators)
